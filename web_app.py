...
/* Target markdown text inside Streamlit containers */
div.stMarkdown p {
    color: #4A3F00 !important;
}

/* ===== CUSTOM RELIABLE FIXES ===== */

/* Parent support text - wrap your text in <div class="parent-support-text">…</div> */
.parent-support-text * {
    color: #4A3F00 !important;  /* dark brown */
    font-weight: 600;
}

/* Slider label text - wrap your label in <div class="slider-text">…</div> */
.slider-text * {
    color: #4A3F00 !important;  /* dark brown */
    font-weight: 600;
}
...
if menu == "Predict GPA":
    ...
    # Study Hours Warning
    if study < 5:
        st.warning("⚠️ Very low study hours. Academic performance may decline.")

    elif study > 50:
        st.warning("⚠️ Extremely high study hours. Risk of burnout.")

    else:
        st.success("✅ Study hours are in a healthy range.")


# Absence Warning
    if absences > 20:
        st.error("🚨 High number of absences. This strongly impacts GPA.")

    elif absences > 10: 
        st.warning("⚠️ Moderate absences detected. Try improving attendance.")

    else:
        st.success("✅ Attendance level is good.")

    tutoring = st.selectbox("Tutoring", [0, 1])
    # Custom label for parental support
    # Custom label
    st.markdown('<p style="color:#4A3F00; font-weight:600;">Parental Support:</p>', unsafe_allow_html=True)

    # Slider without label
    parental = st.slider("", 0, 4)

    # Show value in visible markdown
    st.markdown(f'<p style="color:#4A3F00; font-weight:600;">Selected Value: {parental}</p>', unsafe_allow_html=True)
